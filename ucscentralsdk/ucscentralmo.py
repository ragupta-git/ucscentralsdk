...
from . import ucscentralcoremeta
...
from .ucscentralcoremeta import WriteXmlOption
...
class ManagedObject(UcsCentralBase):
    """
    ...
    def __init__(self, class_id, parent_mo_or_dn=None, **kwargs):
        ...
        UcsCentralBase.__init__(self, ucscentralgenutils.word_u(xml_attribute))

        if self.__parent_mo:
            self.__parent_mo.child_add(self)

        if kwargs:
            for prop_name, prop_value in ucscentralgenutils.iteritems(kwargs):
    ...
    def __set_prop(self, name, value, mark_dirty=True, forced=False):
        ...
        if not forced:
            ...
            if prop_meta.access != ucscentralcoremeta.MoPropertyMeta.READ_WRITE:
                if getattr(self, name) is not None or \
                        prop_meta.access != \
                        ucscentralcoremeta.MoPropertyMeta.CREATE_ONLY:
    ...
    def __str__(self):
        ...
        for prop, prop_value in sorted(ucscentralgenutils.iteritems(
                self.__dict__)):
    ...
    def from_xml(self, elem, handle=None):
        ...
        if elem.attrib:
            if self.__class__.__name__ != "ManagedObject":
                for attr_name, attr_value in ucscentralgenutils.iteritems(
                        elem.attrib):
        ...
        if child_elems:
            for child_elem in child_elems:
                ...
                class_id = ucscentralgenutils.word_u(child_elem.tag)
                child_obj = ucscentralcoreutils.get_ucs_central_obj(class_id,
                                                                    child_elem,
                                                                    self)
    ...
    def show_hierarchy(self, level=0, break_level=None, show_level=None):
        ...
        from .ucscentralcoreutils import print_mo_hierarchy
...
class GenericMo(UcsCentralBase):
    """
    ...
    def __init__(self, class_id, parent_mo_or_dn=None, **kwargs):
        ...
        UcsCentralBase.__init__(self, class_id)

        if kwargs:
            for key, value in ucscentralgenutils.iteritems(kwargs):
    ...
    def to_xml(self, xml_doc=None):
        ...
        if xml_doc is None:
            ...
            xml_obj = SubElement(xml_doc, ucscentralgenutils.word_l(
                self._class_id))
        for key in self.__dict__:
    ...
    def from_xml(self, elem, handle=None):
        ...
        if elem.attrib:
            for name, value in ucscentralgenutils.iteritems(elem.attrib):
        ...
        if children:
            for child in children:
                if not ET.iselement(child):
                    continue
                class_id = ucscentralgenutils.word_u(child.tag)
                pdn = None
                if 'dn' in dir(self):
                    pdn = self.dn
    ...
    def __get_mo_obj(self, class_id):
        ...
        mo_class = ucscentralcoreutils.load_class(class_id)
        ...
        for param in mo_class_params:
            ...
            if mo_param not in self.__properties:
                ...
                np_dict = ucscentralcoreutils.get_naming_props(rn_str,
                                                               rn_pattern)
    ...
    def to_mo(self):
        ...
        from . import ucscentralmeta

        class_id = ucscentralgenutils.word_u(self._class_id)
        if class_id not in ucscentralmeta.MO_CLASS_ID:
            return None
        ...
        for prop in self.__properties:
            if prop in mo.prop_map:
                ...
                UcsCentralWarning("Property %s Not Exist in MO %s" % (
                    ucscentralgenutils.word_u(prop), class_id))
    ...
    def __str__(self):
        ...
        if isinstance(self, GenericMo):
            ...
            for prop, prop_val in sorted(ucscentralgenutils.iteritems(
                    self.__dict__)):
