...
from . ucscentralmeta import MO_CLASS_ID, METHOD_CLASS_ID, \
...
def get_ucs_central_obj(class_id, elem, mo_obj=None):
    ...
    from . import ucscentralmethod
    from . import ucscentralmo

    if class_id in METHOD_CLASS_ID:
        return ucscentralmethod.ExternalMethod(class_id)
    ...
    mo_obj = ucscentralmo.GenericMo(class_id=elem.tag,
...
def load_module(module_name):
    ...
    module_name = ucscentralgenutils.word_u(module_name)
...
def load_class(class_id):
    ...
    class_id = ucscentralgenutils.word_u(class_id)
    if class_id and class_id in MO_CLASS_ID:
        mod_class_id = ucscentralgenutils.word_l(class_id)
...
def write_object(mo_or_list):
    ...
    if isinstance(mo_or_list, ucscentralmethod.ExternalMethod):
        if hasattr(mo_or_list, "out_configs"):
            for child in mo_or_list.out_configs.child:
                if isinstance(child, ucscentralmo.ManagedObject):
                    write_object(child)
    elif isinstance(mo_or_list, list) and len(mo_or_list) > 0:
        for mo in mo_or_list:
            if (isinstance(mo, ucscentralmo.ManagedObject) or
                    isinstance(mo, ucscentralmo.GenericMo)):
                print(mo)
    elif (isinstance(mo_or_list, ucscentralmo.ManagedObject) or
            isinstance(mo_or_list, ucscentralmo.GenericMo)):
        print(mo_or_list)
...
def print_mo_hierarchy(class_id, level=0, break_level=None, show_level=None):
    ...
    if level == 0:
        parents = [ucscentralgenutils.word_u(parent) for parent in
                   MO_CLASS_META[class_id].parents]
        print("[%s]" % (", ".join(sorted(parents))))

    if level == 0 or not show_level or level in show_level:
        print("%s%s" % (level_indent, ucscentralgenutils.word_u(class_id)))
    ...
    if break_level is None or level <= break_level:
        for ch_ in children:
            child = ucscentralgenutils.word_u(ch_)
            if child == class_id:
                continue
...
def _show_tree(class_id, break_level=None, level=0, ancestor_str="",
    ...
    meta_class_id = ucscentralgenutils.word_u(class_id)

    if not ancestor:
        for parent in sorted(MO_CLASS_META[meta_class_id].parents):
            print("[" + ucscentralgenutils.word_u(parent) + "]")
...
def search_class_id(class_id):
    ...
    l_class_id = class_id.lower()
    class_ids = sorted([cid for cid in ucscentralmeta.MO_CLASS_ID
