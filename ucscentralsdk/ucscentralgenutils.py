...
def download_file(driver, file_url, file_dir, file_name):
    ...
    Example:
        driver = UcsCentralDriver()\n
        download_file(driver=UcsCentralDriver(), file_url="http://fileurl",
...
def check_registry_key(java_key):
    ...
    try:
        ...
        UcsCentralWarning("Not able to access registry.")
        return None
...
def get_binary_path(binary):
    ...
    if fpath:
        raise UcsCentralValidationException(
            "Expects only binary name, not Path.")
